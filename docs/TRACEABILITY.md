...
| GitHub App pull-request adapter | IMPLEMENTED | just-in-time App credential resolution, host/repository isolation, bounded REST responses, idempotent existing-PR lookup, and the fail-closed mandatory pull-request description owned by `docs/conventions/pull-requests/description-mandate.md` |
...
| Lifecycle provider error diagnostics | IMPLEMENTED | a failed lifecycle API operation surfaces the provider's own `message` and `errors[].message` in the non-sensitive diagnostic field, length-bounded; no redaction occurs because the channel is secret-free by construction (typed secret-free request, header-isolated credential, provider diagnostic text); the canonical rationale lives in `docs/conventions/hosting-platforms/github/workflows/provider-error-diagnostics.md`; same-package whitebox tests pin every branch including the no-redaction surface |
| Lifecycle fail-closed named-reason diagnostics | VERIFIED | every fail-closed check in the lifecycle payload family emits the named reason — the failed check, the value class, and the remediation pointer — on the lane diagnostic surface, never a secret; the packaging contract tests prove the named form fail-closed on every payload and reject the silent form, including the proven silent default-branch ref check |
| Environment-gate trigger context | IMPLEMENTED | an event-driven caller detects a merged promotion or hotfix on the `pull_request` event without any environment binding and dispatches the environment-gated execution as a `workflow_dispatch` run on the default branch, so the delivery environment's branch policy and required reviewers gate the mutation; the canonical rationale lives in `docs/conventions/hosting-platforms/github/workflows/environment-gate-trigger-context.md`, and the caller copy model rationale lives in `docs/conventions/hosting-platforms/github/workflows/canonical-copies-not-symlinks.md`; packaging contract tests prove the split fail-closed |
...
| Exact lifecycle toolchain provisioning | IMPLEMENTED | every lifecycle `actions/setup-go` location resolves exactly one `toolchain goX.Y.Z` directive from the checked-out tenant `go.mod` and receives the resulting exact version through `go-version`; `go-version-file`, the `go` directive, and latest-patch resolution are rejected |
...
| Dependency-review merge enforcement | VERIFIED | `Dependency review / Dependency admission review` is a required status check for `develop`, `main`, `release/*`, and `support/*` in both ruleset class variants; the repository contract test binds the workflow name and every shared-line ruleset class variant |
| Composite check-context migration | VERIFIED | both shared-line ruleset class variants bind the canonical composite contexts `Quality gates / <os>` per class and `Dependency review / Dependency admission review` per the naming law; the contract test binds the composite contexts for both classes |
| Canonical conformance merge enforcement | IMPLEMENTED | both shared-line ruleset class variants bind the composite context `Canonical conformance / Canonical bindings verification` as a required status check in the canonical sources; the organization re-import and the live verification follow after the merge (GOV-106) |
