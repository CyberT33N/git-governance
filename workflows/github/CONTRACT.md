...
payload by full-length commit SHA. The pinned commit always binds a commit on
the merged develop line of this home (a permanent reference) — a
pull-request-only commit is never a pin, because the cross-repo
reusable-workflow resolver cannot find it. Any deviation from the canonical
caller is a hash mismatch against `caller-hashes.json` and blocks fail-closed
in the fleet's conformance proof. Every caller hash is computed over the
